...
// `Sample` trait brings `.to_sample::<f32>()` into scope for converting our
// i32 samples to the f32 buffers cpal/PipeWire expect.
...
impl AudioRenderer {
    pub(crate) fn new(
        ...
        Self {
            ...
            started: false,
            last_generation: 0,
            last_frame: vec![0i32; format.channels as usize],
            process_callback,
        }
    }
        ...
        if let (Some(cursor_us), Some(sync)) = (cursor_us, self.clock_sync.try_lock()) {
            if let Some(expected_instant) = sync.server_to_local_instant(cursor_us) {
                ...
                if !self.started && playback_instant + early_window < expected_instant {
                    ...
                    let target = self.gain_control.gain();
...
mod tests {
    ...
    fn test_queue_drops_stale_buffers() {
        ...
        let stale_samples: Vec<Sample> = (0..4800 * 2).map(|_| 111i32).collect();
        let fresh_samples: Vec<Sample> = (0..4800 * 2).map(|_| 222i32).collect();
        ...
        assert_eq!(frame_data[0], 222i32);
        assert_eq!(frame_data[1], 222i32);
    }
    ...
    fn test_queue_push_sorts_by_timestamp() {
        ...
        for ts in [300_000i64, 100_000, 200_000] {
            let samples: Vec<Sample> = vec![ts as i32; 48];
            queue.push(AudioBuffer {
                timestamp: ts,
                play_at: Instant::now(),
                samples: Arc::from(samples.into_boxed_slice()),
    ...
    fn test_queue_cursor_advances_correctly() {
        ...
        let num_frames = 480;
        let samples = vec![0i32; num_frames * 2];
    ...
    fn test_cursor_does_not_advance_during_underrun() {
        ...
        let fresh_samples: Vec<Sample> = (0..480 * 2).map(|_| 999i32).collect();
        queue.push(AudioBuffer {
    ...
    fn test_next_frame_no_skip_when_buffer_starts_at_or_after_cursor() {
        ...
        let samples_a: Vec<Sample> = (0..2400 * 2).map(|_| 111i32).collect();
        let samples_b: Vec<Sample> = (0..2400 * 2).map(|_| 222i32).collect();
    ...
    fn test_push_dedup_replaces_overlapping_buffer() {
        ...
        let samples_a: Vec<Sample> = (0..480 * 2).map(|_| 111i32).collect();
        queue.push(AudioBuffer {
            timestamp: 0,
            play_at: Instant::now(),
            samples: Arc::from(samples_a.into_boxed_slice()),
        ...
        let samples_b: Vec<Sample> = (0..480 * 2).map(|_| 222i32).collect();
        queue.push(AudioBuffer {
            timestamp: 5_000,
            play_at: Instant::now(),
            samples: Arc::from(samples_b.into_boxed_slice()),
        ...
        assert_eq!(queue.queue.len(), 1);
        assert_eq!(queue.queue[0].samples[0], 222i32);
    }
    ...
    fn test_push_dedup_no_false_positive_small_chunks() {
        ...
        let samples_a: Vec<Sample> = (0..240 * 2).map(|_| 111i32).collect();
        let samples_b: Vec<Sample> = (0..240 * 2).map(|_| 222i32).collect();
    ...
    fn test_push_dedup_removes_all_overlapping() {
        ...
        let samples_c: Vec<Sample> = (0..960 * 2).map(|_| 333i32).collect();
        queue.push(AudioBuffer {
            timestamp: 9_000,
            play_at: Instant::now(),
        ...
        assert_eq!(queue.queue[0].samples[0], 333i32);
    }
    ...
    fn test_skip_past_entire_buffer_does_not_panic() {
        ...
        let short_samples: Vec<Sample> = (0..48 * 2).map(|_| 111i32).collect();
        let ahead_samples: Vec<Sample> = (0..480 * 2).map(|_| 222i32).collect();
